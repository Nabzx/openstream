...
### 2. Push-to-talk overlay — `electron/overlay/` + `electron/main.js` — **done** ([#300](https://github.com/Nabzx/openstream/issues/300) / [#301](https://github.com/Nabzx/openstream/issues/301)), Liquid in [#358](https://github.com/Nabzx/openstream/issues/358)

The overlay window runs `vibrancy: "hud"`, `visualEffectState: "active"` (it's only ever shown `showInactive()`), `roundedCorners: true`, `hasShadow: true`, `13px` radius to match. **#358** brought it up to the app's Liquid look, a release behind: `overlay.css` now uses the same near-clear film as the app cards (`--screen` at `rgba(255,255,255,0.09)`, `blur(8px) saturate(1.7)`, `--rim`, one soft drop shadow) instead of the old blue wash and inner glow. The status word is the system sans in sentence case, the `>` prefix and the blinking cursor are gone, the held-result panel matches the app's inset and buttons, and only the held text stays mono. The waveform is taller, drawn in `--acc`, and rides a slow ripple (`overlay.js` `idleLevel()`, `prefers-reduced-motion`-aware) while the mic is quiet.
...
### 5. Landing page — `site/index.html` — **done**, Liquid in [#363](https://github.com/Nabzx/openstream/issues/363)

A single self-contained HTML file. It stays its **own surface**, not a port: dark by design, its own sharper web-native character, but the palette, the type discipline and the voice now match the app. Dark navy ground (`#0A1420`), the `#8FC4FF` accent, the system sans for the headline and prose, JetBrains Mono kept for the wordmark, the hero demo terminal and the install block. No glow text-shadows.

Structure: a 100vh hero with the looping demo terminal (which now shows the break-safety behaviour, not a film quote), then `why` (the 40-vs-150 wpm bars), `what`, `install`, `status`. [#365](https://github.com/Nabzx/openstream/issues/365) dropped the terminal costume: the fixed bright-blue status bar, the scanline, the `──` on section labels, the `>` on the install button, the forced lowercase (the `~ openstream` wordmark stays lowercase). Google Fonts is allowed here (not offline-first, unlike the app). British spelling, no em or en dashes.
...
- **Liquid** *(#350, post-launch)*: #341 was cleaner but still generic. Moved to a near-clear material (bright film panels, low blur, refracting rim, softer `#8FC4FF`), redrew the mark as a glass tile, added the reduced-transparency fallback, and rebuilt Settings as one repeated block. Chosen from five blue-glass mocks. Follow-up: the window vibrancy went `"under-window"` to `"hud"` because the dark background material read as flat blue-grey under the clear film; a painted in-app gradient behind the glass was tried and dropped in favour of the real desktop showing through.
- **Overlay to Liquid** *(#358, post-launch)*: the overlay onto the app's near-clear material, the terminal decoration gone (`>`, blinking cursor, lowercase, `[ ]` buttons), the waveform taller and drawn in the accent with a quiet idle ripple.
- **Landing to Liquid** *(#363, post-launch)*: the landing page's palette, type and voice brought in line with the app (`#8FC4FF`, system sans for reading, sentence case), the terminal costume dropped (status bar, scanline, `──`, `>`). Kept as its own dark web-native surface, not a port. Its hero demo now shows the break-safety behaviour instead of a film quote.
...
- **Overlay**: native `vibrancy: "hud"` + rounded + shadow; CSS glass on top. [#358](https://github.com/Nabzx/openstream/issues/358) brought it to the app's Liquid material and dropped the terminal decoration (the `>` prefix, the blinking cursor, lowercase, `[ ]` buttons). True Liquid Glass is still a post-launch native rewrite ([#301](https://github.com/Nabzx/openstream/issues/301)).
