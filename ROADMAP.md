...
- [ ] ~~Support multiple whisper.cpp model sizes; settings toggle for speed vs. accuracy~~ - **removed by #30.** One model. Was `ggml-base.en.bin`; #308 swapped it for `large-v3-turbo` (q5_0, ~547 MiB) - turbo is near-`large-v3` accuracy at roughly `small.en` speed, so it clears the old "bigger model blows the sub-1s budget" objection (to be confirmed by the #228 latency pass)
...
- **Parakeet TDT 0.6B v3 as the transcription model server** ([#176](https://github.com/Nabzx/openstream/issues/176), [#204](https://github.com/Nabzx/openstream/issues/204), [#205](https://github.com/Nabzx/openstream/issues/205)) — replace the whisper transcription model (`large-v3-turbo` since #308). Pipeline shape unchanged ([ADR-0002](docs/adr/0002-no-one-model-dictation-engine.md)); the gate is proving a local runtime on Apple Silicon. OpenSuperWhisper already ships Parakeet via FluidAudio's Swift/CoreML package.
